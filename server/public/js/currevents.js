...
function resetSeatFlow() {
  ...
  if (selectedSeats.length > 0 && activeSeatEventId) {
    clearReservation();
  }
  
  wantedCount = 0; selectedSeats = []; reservedSeats = []; activeSeatEventId = null;
  if (reservationTimer) {
...
document.getElementById('proceedToSeats').addEventListener('click', () => {
  ...
  const ev = allEvents.find(e => e.id === activeSeatEventId);
  if (!ev) return;
...
async function updateReservation() {
  ...
  reservationDebounce = setTimeout(async () => {
    try {
      const res = await fetch(`${API}/events/${activeSeatEventId}/reserve`, {
...
document.getElementById('confirmBooking').addEventListener('click', async () => {
  ...
  try {
    const res  = await fetch(`${API}/events/${activeSeatEventId}/enroll`, {
      method: 'POST', headers: H(),
      body: JSON.stringify({ seats: selectedSeats }),
    });
    const data = await res.json();
...
async function quickEnroll(eventId) {
  ...
  try {
    const res  = await fetch(`${API}/events/${eventId}/enroll`, {
      method: 'POST', headers: H(), body: JSON.stringify({ seats: [] }),
    });
    const data = await res.json();
