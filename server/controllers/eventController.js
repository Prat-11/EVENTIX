...
export const reserveSeats = async (req, res, next) => {
  try {
    ...
    req.app.get('io')?.to(`event:${req.params.id}`).emit('seats:reserved', {
      eventId: result.eventId, seats: result.seats, userId: result.userId, expiresAt: result.expiresAt
    });
...
export const clearReservation = async (req, res, next) => {
  try {
    ...
    if (result.seats.length > 0) {
      req.app.get('io')?.to(`event:${req.params.id}`).emit('seats:released', { eventId: result.eventId, seats: result.seats });
    }
